...
class IntrusionDetector:
    ...
    def __init__(self, active_learner, classifier, initial_train_data, config_params):
        ...
        self.retrain_mode = config_params['retrain_mode']
        self.retrain_sample_count = config_params['retrain_sample_count']

        train_data_x, train_data_y, self.label_encoder = initial_train_data

        self.backup_train_data_x = train_data_x
        self.backup_train_data_y = train_data_y
    ...
    def predict(self, x, data_ids):
        self.add_to_input_data_ids(data_ids)

        probabilities_list = self.classifier.predict(x)
        ...
        for i in range(len(probabilities_list)):
            ...
            predicted_class_labels = np.append(predicted_class_labels, [predicted_class_label])
            class_probs = np.append(class_probs, [class_prob])

            if not self.is_enough_confidence(probabilities):
                self.add_to_unlabeled_data_pool(x[i], data_ids[i])
    ...
    def add_to_input_data_ids(self, data_ids):
        self.input_data_ids = np.append(self.input_data_ids, data_ids)

    def complete_train_data_randomly(self):
        selected_train_data_x = np.empty((0, 78))
        selected_train_data_y = np.array([])

        for c in self.train_classes:
            indices = np.argwhere(self.train_data_y == c)
            new_sample_count = np.size(indices)

            indices = np.argwhere(self.backup_train_data_y == c)
            old_sample_count = np.size(indices)

            n = self.retrain_sample_count - new_sample_count
            if n > old_sample_count:
                n = old_sample_count

            selected_indices = np.random.choice(indices.flatten(), n, replace=False)

            selected_train_data_x = np.append(selected_train_data_x, self.backup_train_data_x[selected_indices], axis=0)
            selected_train_data_y = np.append(selected_train_data_y, self.backup_train_data_y[selected_indices])

        return selected_train_data_x, selected_train_data_y
    ...
    def update_train_data(self, labeled_data_x, labeled_data_y):
        ...
        if curr_train_data_count > 0:
            update_success = True

        completed_data_x, completed_data_y = self.complete_train_data_randomly()

        self.backup_train_data_x = np.append(self.backup_train_data_x, self.train_data_x, axis=0)
        self.backup_train_data_y = np.append(self.backup_train_data_y, self.train_data_y)

        self.train_data_x = np.append(self.train_data_x, completed_data_x, axis=0)
        self.train_data_y = np.append(self.train_data_y, completed_data_y)
    ...
    def transfer_data_from_rare_to_train(self):
        ...
        for i in range(classes.size):
            if counts[i] > self.new_class_min_count_th:
                ...
                if self.retrain_mode == "partial_old_data":
                    self.backup_train_data_x = np.append(self.backup_train_data_x, new_train_data_x, axis=0)
                    self.backup_train_data_y = np.append(self.backup_train_data_y, new_train_data_y)
