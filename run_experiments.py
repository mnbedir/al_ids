...
def train_model(model, exp_params, datasets, is_first_time=True):
    ...
    # Save the best trained models (in case we need to continue training from this point on)
    # We assume that the models classes will restore the best weights (epoch with lowest validation error) at end of training
...
def run_experiment_actions(model_origin, model_type, model_name, action, resources):
    ...
    if model_origin == 'new':
        ...
        logging.info('Loading models from file: {}'.format(model_location))
        ...
        if 'model2_location' in exp_params:
            ...
            logging.info('Loading models 2 from file: {}'.format(model2_location))
...
def main():
    ...
    filename = "experiment_specs/test.csv" #get_filename_from_args()
