...
from intrusion_detector_fast2 import IntrusionDetector
...
def split_initial_data(datasets_orig, n):
    (X_train, y_train), (X_test, y_test) = datasets_orig

    indices = np.argwhere(y_train.to_numpy() == 'PortScan')
    selected_indices_1 = np.random.choice(indices.flatten(), n, replace=False)

    indices = np.argwhere(y_train.to_numpy() == 'DDoS')
    selected_indices_2 = np.random.choice(indices.flatten(), n, replace=False)

    indices = np.argwhere(y_train.to_numpy() == 'BENIGN')
    selected_indices_3 = np.random.choice(indices.flatten(), n, replace=False)

    selected_indices = np.append(selected_indices_1, selected_indices_2)
    selected_indices = np.append(selected_indices, selected_indices_3)

    X_train_i_s = X_train.to_numpy()[selected_indices]
    y_train_i_s = y_train.to_numpy()[selected_indices]

    X_train_r = X_train.drop(X_train.index[selected_indices], axis=0)
    y_train_r = y_train.drop(y_train.index[selected_indices])
...
def run_experiment(exp_config, classifier_config, al_config, ids_config):
    ...
    # X_test, y_test = prepare_attack_dataset((X_test, y_test))
    ...
    initial_datasets_orig, remaining_datasets_orig = split_initial_data(datasets_orig,
                                                                        ids_config['retrain_sample_count'])
...
def main():
    ...
    config_file_path = sys.argv[1]  # "default_config2.txt"
